...
use unm_resolver::{engine::{Engine, Artist, Context}, resolve::Song};
...
pub async fn engine_example_wrapper(engine: impl Engine) {
    ...
    println!("[Retrieved] 周杰伦 - 青花瓷: {} (from {})", retrieved_result.url, retrieved_result.source);
    println!("Search taken {:?} while retrieve took {:?}.", search_time_taken, retrieve_time_taken);
}
