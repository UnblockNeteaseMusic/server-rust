...
impl BilibiliProvider {
    ...
    async fn search(&self, info: &SongMetadata) -> ProvidersResult<Option<i64>> {
        let url_str = format!(
    ...
    async fn track(&self, id: i64) -> ProvidersResult<Option<String>> {
        let url_str = format!(
...
impl Provide for BilibiliProvider {
    async fn check(&self, info: &SongMetadata) -> ProvidersResult<Option<String>> {
