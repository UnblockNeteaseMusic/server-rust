...
static SECRET_KEY: &'static [u8; 8] = b"ylzsxkwm";
static ARRAY_MASK: &'static [u64; 64] = &[
...
static ARRAY_IP: [i32; 64] = [57, 49, 41, 33, 25, 17, 9, 1, 59, 51, 43, 35, 27, 19, 11, 3, 61, 53, 45, 37, 29, 21, 13, 5, 63, 55, 47, 39, 31, 23, 15, 7,
    56, 48, 40, 32, 24, 16, 8, 0, 58, 50, 42, 34, 26, 18, 10, 2, 60, 52, 44, 36, 28, 20, 12, 4, 62, 54, 46, 38, 30, 22, 14, 6];
static ARRAY_E: [i32; 64] = [31, 0, 1, 2, 3, 4, -1, -1, 3, 4, 5, 6, 7, 8, -1, -1, 7, 8, 9, 10, 11, 12, -1, -1, 11, 12, 13, 14, 15, 16, -1, -1, 15, 16, 17,
...
static ARRAY_P: [i32; 32] = [15, 6, 19, 20, 28, 11, 27, 16, 0, 14, 22, 25, 4, 17, 30, 9, 1, 7, 23, 13, 31, 26, 2, 8, 18, 12, 29, 5, 21, 10, 3, 24];
static ARRAY_IP1: [i32; 64] = [39, 7, 47, 15, 55, 23, 63, 31, 38, 6, 46, 14, 54, 22, 62, 30, 37, 5, 45, 13, 53, 21, 61, 29, 36, 4, 44, 12, 52, 20, 60, 28, 35, 3, 43, 11, 51, 19, 59, 27, 34, 2, 42, 10, 50, 18, 58, 26, 33, 1, 41, 9, 49, 17, 57, 25, 32, 0, 40, 8, 48, 16, 56, 24];
static ARRAY_PC1: [i32; 56] = [56, 48, 40, 32, 24, 16, 8, 0, 57, 49, 41, 33, 25, 17, 9, 1, 58, 50, 42, 34, 26, 18, 10, 2, 59, 51, 43, 35, 62, 54, 46, 38, 30, 22, 14, 6, 61, 53, 45, 37, 29, 21, 13, 5, 60, 52, 44, 36, 28, 20, 12, 4, 27, 19, 11, 3];
static ARRAY_PC2: [i32; 64] = [13, 16, 10, 23, 0, 4, -1, -1, 2, 27, 14, 5, 20, 9, -1, -1, 22, 18, 11, 3, 25, 7, -1, -1, 15, 6, 26, 19, 12, 1, -1, -1, 40, 51, 30, 36, 46, 54, -1, -1, 29, 39, 50, 44, 32, 47, -1, -1, 43, 48, 38, 55, 33, 52, -1, -1, 45, 41, 49, 35, 28, 31, -1, -1];
...
fn des_sub_keys(key: u64, k: &mut [u64; 16], mode: Mode) {
    let mut temp: u64 = 0;
    /* PC-1变换 */
    temp = bit_transform(&ARRAY_PC1, key);
    for j in 0..16 {
        {
            /* 循环左移 */
            let source = temp;
...
fn encrypt_private(src: &[u8], key: &[u8; 8]) -> Vec<u8> {
    ...
    let mut key_l: u64 = 0;
    for i in 0..8 {
        key_l |= (key[i] << (i * 8)) as u64;
    }
    let num = src_len >> 3;
    // 子密钥（临时数据）
    let mut sub_key = [0u64; 16];

    des_sub_keys(key_l, &mut sub_key, Mode::Decrypt);

    // 加密
    let mut pSrc = vec![0u64; num];
    for i in 0..num {
        for j in 0..8 {
            pSrc[i] |= (src[i * 8 + j] as u64) << (j * 8)
        }
    }
    let pEncyrptLength = ((num + 1) * 8 + 1) / 8;
    //存放密文
    let mut pEncyrpt = vec![0u64; pEncyrptLength];
    // 计算前部的数据块(除了最后一部分)
    for i in 0..num {
        pEncyrpt[i] = des64(&sub_key, pSrc[i]);
    }

    // 保存多出来的字节
    let szTail = src[num * 8..].to_vec();

    let mut tail64 = 0u64;
    // 处理结尾处不够8个字节的部分
    let tailNum = src_len % 8;
    for i in 0..tailNum {
        tail64 = tail64 | ((szTail[i] as u64) << (i * 8))
    }
    // 计算多出的那一位(最后一位)
    pEncyrpt[num] = des64(&sub_key, tail64);
    let mut result = vec![0u8; pEncyrpt.len() * 8];
    let mut temp = 0;
    let ff: u64 = 255;
    let mut enc: u64;
    // 将密文转为字节型
    for i in 0..pEncyrpt.len() {
        for j in 0..8 {
            enc = ff & (pEncyrpt[i] << (j * 8));
...
pub fn encrypt(src: &[u8]) -> Vec<u8> {
	return encrypt_private(&src, &SECRET_KEY);
}
